...
import com.onsemi.dots.model.UserGroupAccess;
...
public class UserGroupAccessDAO {
    ...
    public List<UserGroupAccess> getGroupAccess(String groupId) {
        String sql = "SELECT * FROM dots_menu_access WHERE group_id = '" + groupId + "'";
    ...
    public List<UserGroupAccess> getUserGroupAccess(String groupId) {
        String sql = "SELECT m.id AS menu_id, m.parent_code, m.code, m.name, uga.id, uga.group_id, IF (m.id = uga.menu_id, \"checked=\\\"\\\"\", \"\") AS selected "
                + "FROM dots_menu m LEFT JOIN dots_menu_access uga ON uga.group_id = '" + groupId + "' AND m.id = uga.menu_id ORDER BY m.code";
        List<UserGroupAccess> userGroupAccessList = new ArrayList<UserGroupAccess>();
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
            UserGroupAccess userGroupAccess;
    ...
    public QueryResult addAccess(String groupId, String menuId) {
        ...
        String sql = "INSERT into dots_menu_access (group_id, menu_id) "
                + "SELECT * FROM (SELECT ? AS group_id, ? AS menu_id) AS tmp "
                + "WHERE NOT EXISTS (SELECT id FROM dots_menu_access WHERE group_id = ? AND menu_id = ?) "
                + "LIMIT 1";
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
    ...
    public QueryResult removeAccess(String groupId, String[] groupAccess) {
        ...
        try {
            for (String access : groupAccess) {
                String findId = "SELECT id FROM dots_menu_access WHERE group_id = ? AND menu_id = ?";
            ...
            String delete = "DELETE FROM dots_menu_access WHERE id NOT IN (" + idList + ") AND group_id ='" + groupId + "'";
            if (idList.equals("")) {
                delete = "DELETE FROM dots_menu_access WHERE group_id = '" + groupId + "'";
            }
    ...
    public QueryResult removeAccessByGroupId(String groupId) {
        ...
        try {
            PreparedStatement ps = conn.prepareStatement(
                    "DELETE FROM dots_menu_access WHERE group_id = '" + groupId + "'"
            );
            queryResult.setResult(ps.executeUpdate());
            ps.close();
        } catch (SQLException e) {
