...
public class MenuDAO {
    ...
    public List<Menu> getMenuAccess(String parentCode, String currentPath, String groupId) {
        String sql = "SELECT m.parent_code, m.code, m.name, m.url_path, m.target, m.icon, m.label_key, "
                + "IF(url_path = \"" + currentPath + "\", \"class=\\\"active\\\"\", \"\") AS class_active FROM dots_menu m, dots_menu_access ma "
    ...
    public List<Menu> getMenuList(String parentCode) {
        String sql = "SELECT id, parent_code, code, name, icon, label_key FROM dots_menu m WHERE m.parent_code='" + parentCode + "' ORDER BY m.sequence ASC";
        List<Menu> menuList = new ArrayList<Menu>();
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
            Menu menu;
    ...
    public Boolean menuExist(String path) {
        String sql = "SELECT id FROM dots_menu m WHERE m.url_path='" + path + "'";
        Boolean exist = false;
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
